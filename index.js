...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.log("Помилка створення нового масиву з переданих елементів");
    return [];
  }
}
...
function findElementIndexes(arr, element) {
  ...
  let lasttIndex = arr.lastIndexOf(element);
  let newArr = [];
  if (firstIndex !== -1 && lasttIndex > firstIndex) {
    newArr.push(firstIndex, lasttIndex);
  } else {
...
function customUnshift(arr, ...elements) {
  ...
  if (!Array.isArray(arr)) {
    return "Вхідний параметр не є масивом";
  }
...
function customIncludes(arr, element) {
  ...
  const count = arr.filter((arrElement) => {
    return arrElement === element;
  }).length;
  let result = (console.log(count), elem);
  return result;
}
